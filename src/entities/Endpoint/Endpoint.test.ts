...
describe('Entity - Endpoint', () => {
  ...
  it('it should authenticate with authAdmin middleware', async () => {
    ...
    const endpoint = new Endpoint({
      ...
      authParams: { permission },
      handler: (req: Request, res: Response) => {
        res.sendStatus(200);
      }
    } as EndpointOptions<EndpointAuthType.ADMIN>);
  ...
  it('it should authenticate with authAdmin middleware and check permission', async () => {
    const email = faker.internet.email();
    const permission = faker.lorem.word();
    const token = generateAccessToken(
      email,
  ...
  it('it should authenticate with authAdmin middleware and check permissions', async () => {
    const email = faker.internet.email();
    const permissions = [
      faker.lorem.word(),
      faker.lorem.word(),
    ...
    const endpoint = new Endpoint({
      ...config,
      authType: EndpointAuthType.ADMIN,
      authParams: { permission: permissions[1] },
      handler: (req: Request, res: Response) => {
        res.sendStatus(200);
      }
    } as EndpointOptions<EndpointAuthType.ADMIN>);
